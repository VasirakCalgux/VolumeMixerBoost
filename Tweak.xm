...
static void registerApp(){
    ...
    dispatch_once(&onceToken, ^{
        ...
        MRYIPCCenter *idClient = [MRYIPCCenter centerNamed:@"com.vasirakcalgux.volumemixerboost/register"];

        int pid = NSProcessInfo.processInfo.processIdentifier;
        [idClient callExternalVoidMethod:@selector(register:) withArguments:@{@"bundleID":bundleID, @"pid":@(pid)}];

        NSString *appNotify = [NSString stringWithFormat:@"com.vasirakcalgux.volumemixerboost/%@~%d/setVolume", bundleID, pid];
...
%ctor{
    ...
    loadPref();
    int token;
    notify_register_dispatch("com.vasirakcalgux.volumemixerboost/loadPref", &token, dispatch_get_main_queue(), ^(int token) {
        loadPref();
    });
}
